...
```
SCOUT (no AI, pure JS scoring) → RECON (grok-4-fast) → GUARDIAN preflight → TAILOR (grok-4) → LEDGER (no AI) → COACH (grok-4, on stage change / on demand)
```
...
- Model is resolved per-agent from `orbitapply.json` (`agents.overrides[id].model` → `agents.defaults.model`). `src/utils/constants.js` `AGENT_MODELS` documents the intended mapping (ORBI/TAILOR/COACH = grok-4; rest = grok-4-fast). All agent calls go through xAI's Grok API (`GROK_API_KEY`, OpenAI-compatible `/chat/completions`), not Anthropic.
...
- `src/utils/searchProvider.js` — 8-provider waterfall (Tavily → Perplexity → Brave → SerpAPI → Bing → Google → Jina → DuckDuckGo). Fallback order is intentional; DuckDuckGo needs no key.
